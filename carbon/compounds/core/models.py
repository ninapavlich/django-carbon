...
from carbon.utils.template import get_page_templates
...
class Template(VersionableAtom, TitleAtom):
    ...
    def save(self, *args, **kwargs):
        ...
        if self.custom_template is not None and self.custom_template != '':
            self.file_template = None

        if original is not None and original.custom_template != self.custom_template:
            template_content_changed = True

        super(Template, self).save(*args, **kwargs)

        if template_content_changed is True:
...
class BaseFrontendPackage(VersionableAtom, TitleAtom):
    ...
    def render(self, save_file=True, force=False):
        ...
        if force is False and self.needs_render is False and self.error_source_content is None:
            return
        ...
        if success:
            ...
            if is_different or force is True:
    ...
    def get_url(self, minified):
        if self.peg_revision is None or settings.DEBUG:
            if minified:
...
class BaseFrontendResource(VersionableAtom, TitleAtom, OrderedItemAtom):
    ...
    def store_raw_file(self):
        ...
        if success:
            ...
            skip_file_write = self.is_source_package() is True and self.is_source_file() is False
    ...
    def delete_downloaded_files(self):
        if self.file_source_url is not None:
...
def delete_css_file(sender, instance, *args, **kwargs):
    if instance.parent:
        instance.parent.request_render()


@receiver(models.signals.post_delete, sender=JSResource)
def delete_js_file(sender, instance, *args, **kwargs):
    if instance.parent:
        instance.parent.request_render()
