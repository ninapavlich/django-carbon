...
def is_incremented_file(original, updated):
    ...
    try:
        ...
        if are_same_start_path is False:
            return False
        ...
        if increments_are_integers is False:
            return False
...
def _media_file_name(instance, filename, file_attribute_name, folder, model_name):
    ...
    if instance.pk:
        ...
        if full_path != current_path:
            if is_incremented_file(full_path, current_path) is False:
    ...
    if exists:
        ...
        if exists_in_separate_object is False and instance.allow_overwrite is True:
...
def base_remove_image_file_from_s3(sender, instance, using, model_name, storage_bucket_path, **kwargs):
    ...
    if exists_in_separate_object is False and delete_file_on_delete:
